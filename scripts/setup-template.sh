...
replace_in_file() {
    ...
    if [ -f "$file" ]; then
        ...
        sed -i "s|Addeus|$COMPANY_NAME|g" "$file"
        sed -i "s|4 rue du marché 63140 Châtel-Guyon|$COMPANY_ADDRESS|g" "$file"
        sed -i "s|contact@spicy-vs-sweet.com|$SUPPORT_EMAIL|g" "$file"
        sed -i "s||$TWITTER_HANDLE|g" "$file"
