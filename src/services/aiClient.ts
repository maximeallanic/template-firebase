...
export const generateGameQuestions = async (input: GameGenerationInput): Promise<GameGenerationOutput> => {
    ...
    console.log('[AI-CLIENT] 📤 Calling generateGameQuestions', {
        phase: input.phase,
    ...
    try {
        ...
        if (!result.data?.data) {
            console.warn('[AI-CLIENT] ⚠️ Response has no data!', result);
        }

        return result.data;
    } catch (error) {
        const duration = Math.round(performance.now() - startTime);
        console.error('[AI-CLIENT] ❌ Error after', `${duration}ms:`, {
...
export const generateWithRetry = async (
    ...
    console.log('[AI-CLIENT] 🎲 generateWithRetry started', {
        phase: input.phase,
        topic: input.topic || '(auto)',
        difficulty: input.difficulty || 'normal',
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.log(`[AI-CLIENT] ✅ Success on attempt ${attempt}/${maxRetries} (total: ${totalDuration}ms)`);
            ...
            console.warn(`[AI-CLIENT] ⚠️ Attempt ${attempt}/${maxRetries} failed:`, {
            ...
            if (errorCode && NON_RETRYABLE_ERRORS.includes(errorCode)) {
                const totalDuration = Math.round(performance.now() - retryStartTime);
                console.error(`[AI-CLIENT] ❌ Non-retryable error (${errorCode}), stopping retries`, {
            ...
            if (attempt < maxRetries) {
                ...
                console.log(`[AI-CLIENT] ⏳ Waiting ${backoffMs}ms before retry...`);
    ...
    const totalDuration = Math.round(performance.now() - retryStartTime);
    console.error('[AI-CLIENT] ❌ All retries exhausted', {
        attempts: maxRetries,
        totalDuration: `${totalDuration}ms`,
