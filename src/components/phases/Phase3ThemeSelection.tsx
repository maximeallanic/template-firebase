...
export const Phase3ThemeSelection: React.FC<Phase3ThemeSelectionProps> = ({
    ...
    // Which team is selecting next?
    const selectingTeamCount = Object.keys(currentSelections).length;
    const selectingTeam = selectionOrder[selectingTeamCount];
    const isBothTeamsDone = selectingTeamCount >= 2;

    // Check if current user is on the selecting team
    const isOnSelectingTeam = currentTeam === selectingTeam;
    ...
    const handleThemeSelect = async (originalIndex: number) => {
        ...
        try {
            ...
            // Play success sound on successful selection
    ...
    return (
        <div className="flex flex-col items-center justify-center p-6 space-y-6 w-full max-w-4xl mx-auto">
            ...
            {/* Waiting Message for non-selecting team */}
            {!isOnSelectingTeam && (
                <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
